...
class Profile extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    let { profile, isLoadedProfile, errorMessage } = this.props.user;
    ...
    if (prevProps.user.isUpdatingProfile && this.props.user.isUpdatingProfile && this.props.errorMessage) {
  ...
  handleUpdateProfile = async (e) => {
    ...
    if (avatarUpload) {
      ...
      formData.append('canu_avatar', avatarUpload);
      let res = await uploadImage(formData);
      if (res.status) {
        console.log(res.data);
        avatar = res.data.canu_avatar[0].url;
      } else {
        this.setState({
                  ...
                  <div className="row">
                    <div className="col-xl-4">
                      <div className="submit-field">
                        ...
                        <input onChange={this.handleChangePassword} value={currentPassword} autocomplete="new-password" name={`currentPassword-${Math.random()}`} autocomplete="false" type="password" className="with-border"/>
                      </div>
                    </div>
