...
const _titlePage = 'List How To Use';
class CmsHowToUse extends React.Component {
	...
	getOption = () => {
		this.setState({
			...
			let _res = await OtherServices.getListGuide({
	...
	callAPIDelete = async () => {
		...
		let _res = await OtherServices.deleteGuide(this.state?.currentPostDelete?.id);
		if(_res?.status){
			this.setState({
				page: 0,
				currentPostDelete: false,
	...
	render(){
		...
		return(
			<React.Fragment>
			<div className="Guide-cms-content">
				<div className='cms-page-content' id='services-list-add-new'> 
					<div className='btn-create-new-service'>
						<div className="row">
							...
							<div className='col-3'>
								<Link to={`${UrlPath.CmsPath.CmsNewHowToUse}`}>
						...
						{this.state.dataList.map(x => {
							return(
								<li>
									...
									<div className='descriptions'>{x.description}</div>
									<div className='createdAt'>{x.createdAt}</div>
									<div className='actions'>
										<Link to={`${UrlPath.CmsPath.CmsHowToUse}/${x.id}`} className='btn-update' title='update'><i className='icon-material-outline-launch'></i></Link>
