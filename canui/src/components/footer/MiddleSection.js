...
export default function MiddleSection (props){
	...
	const _isLogin = useSelector((state) => state?.user?.profile?.id);
		...
		return (
			<div key={`footer-middleAccount`} className="col-xl-2 col-lg-2 col-md-3">
				<div className="footer-links">
					<h3>{t('Account')}</h3>
					<ul>
						{checkAuthenticated() && <li key="account1"><Link to={`${UrlPath.MyPage}`} ><span>{t('My Page')}</span></Link></li>}
						{!checkAuthenticated() && <li key="account2"><a href="#sign-in-dialog" onClick={e => handleClickOnLogin(e)} className="popup-with-zoom-anim log-in-button"><span>{t('Login')}</span></a></li>}
					</ul>
				</div>
			</div>
		);
