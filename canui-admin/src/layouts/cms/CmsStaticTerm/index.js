...
class CmsStaticTerm extends React.Component {
	constructor(props) {
		super(props);
	...
	callAPiUpdateProperties = async () => {
		...
		if(this.state.banner_file){
			let formData = new FormData();
      		formData.append('BannerImagesTerm', this.state.banner_file);
			let _resImage;
			try{
			...
			if(_resImage?.status && _resImage?.data){
				this.setState({
					banner_url: _resImage.data.BannerImagesTerm[0].url,
					banner_file: false,
				});
			}
		}
		...
		let _res;
		try{
			_res = await setOptionPageTerms({objectValue: _data});
		}catch(err){
