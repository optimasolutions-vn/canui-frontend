...
const _titlePage = 'Create How To Use';
function createMarkup(content){
	return{__html: content};
}
class CmsNewHowToUse extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
			data: {},
		...
		_list.map(x => {
			if(this.state[`${x.toLowerCase()}`] && this.state[`${x.toLowerCase()}`]?.title?.length > 0 && this.state[`${x.toLowerCase()}`]?.content){
				_dataPre.push({
					...
					description: this.state[`${x.toLowerCase()}`]?.description,
	...
	callAPiUpdate = () => {
		...
		if(_data.length > 0){
			_data.map(x => {
				_requestList.push(OtherServices.UpdateGuide(x));
			})
			Promise.all([..._requestList])
				.then(res => {
					...
					this.setState({..._n});
					this.props.history.push(`${UrlPath.CmsPath.CmsHowToUse}`)
				})
				.catch(err => {
					console.log(err);
	...
	onChangeDes = (_x, e) => {
		e.preventDefault();
		let _obj = this.state[_x.toLowerCase()];
		_obj.description = e.target.value;
		this.setState({
			[`${_x.toLowerCase()}`]: _obj
		})
	}
					...
					<div className="row">
						<div className="col-12">
							<div className="submit-field">
								<h5>{t('Title')}</h5>
								<input className='with-border' defaultValue='' type="text" name={`title_${x.toLowerCase()}`} onChange={e => this.onChangeTitle(`${x.toLowerCase()}`, e)} />
							</div>
							<div className="submit-field">
								<h5>{t('Description')}</h5>
								<textarea className='with-border' defaultValue='' type="text" name={`description_${x.toLowerCase()}`} onChange={e => this.onChangeDes(`${x.toLowerCase()}`, e)} />
							</div>
							<div className="submit-field">
								<h5>{t('Content')}</h5>
								<Editor
